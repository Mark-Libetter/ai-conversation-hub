...
EXTRA_SOURCES = ("claude", "codepilot", "cursor", "marvis", "qclaw", "qoderwork")
...
def default_candidates(source: str) -> list[Path]:
    ...
    if source == "codepilot":
        return [
            ...
            application_support / "CodePilot" / "codepilot.db",
        ]
    if source == "cursor":
        return [application_support / "Cursor" / "User" / "globalStorage"]
    if source == "marvis":
        roots = list(
            (application_support / "Tencent" / "Marvis" / "User").glob("*/database/data.db")
        )
        roots.extend(
            (application_support / "Marvis" / "User").glob("*/database/data.db")
        )
        return sorted(roots, key=lambda path: path.stat().st_mtime if path.exists() else 0, reverse=True)
    if source == "qclaw":
        return [home / ".qclaw"]
    if source == "qoderwork":
        return [
...
def estimate_conversations(source: str, path: Path | None) -> int:
    ...
    try:
        if source == "claude":
            transcript_ids = {file_path.stem for file_path in _claude_transcript_files(path)}
            return len(
                transcript_ids
        ...
        if source == "cursor":
            with readonly_db(path / "conversation-search.db") as conn:
                return int(
                    conn.execute(
        ...
        if source == "qclaw":
            index = path / "agents" / "main" / "sessions" / "sessions.json"
            sessions = json_value(index.read_text(encoding="utf-8"), {})
            return sum(
                1
                for key in sessions
                if not any(
...
def _candidate_filenames(source: str) -> tuple[str, ...]:
    return {
        ...
        "marvis": ("data.db",),
        "qoderwork": ("agents.db",),
        "cursor": ("conversation-search.db", "state.vscdb"),
        "qclaw": ("sessions.json",),
    ...
    for root in roots:
        ...
        for current, dirs, files in os.walk(root):
            ...
            for filename in files:
                ...
                if source in {"claude", "qclaw"}:
                    ancestors = list(candidate.parents)
                    for ancestor in ancestors[:6]:
                        if validate_source(source, ancestor)[0]:
                            return ancestor.resolve()
                elif source == "cursor":
                    parent = candidate.parent
                    if validate_source(source, parent)[0]:
                        return parent.resolve()
                elif validate_source(source, candidate)[0]:
        ...
        for current, dirs, files in os.walk(root):
            ...
            for filename in files:
                ...
                for source in tuple(possible):
                    ...
                    if source in {"claude", "qclaw"}:
                        candidates = list(file_path.parents)[:6]
                    elif source == "cursor":
